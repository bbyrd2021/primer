...
async def upload_papers(
    ...
    for file in files:
        if not file.filename or not file.filename.lower().endswith(".pdf"):
            continue
        tasks.append(_process_single_file(file, research_question, session_id, session_dir))
...
async def _process_single_file(
    ...
    card_task = loop.run_in_executor(
        None, extract_paper_card, full_text, file.filename, research_question, session_id
    )
